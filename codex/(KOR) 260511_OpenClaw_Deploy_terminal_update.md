...
기본 주소 `<WEB_SERVER_DOMAIN>`는 계속 Nginx Proxy Manager를 바라보도록 유지했다.
...
- 외부 WebSocket URL: `wss://<WEB_SERVER_OPENCLAW_DOMAIN>`
...
`origin not allowed` 에러는 위 `allowedOrigins`에 `https://<WEB_SERVER_OPENCLAW_DOMAIN>`를 추가하고 Gateway를 재시작해서 해결했다.
